...
### เริ่ม backend API (เฉพาะตอนทำ API test)

ต้องมี Docker Desktop แล้วรัน container ของโจทย์:

```bash
docker run -d --rm --name qa-practice-api -p8887:8081 rvancea/qa-practice-api:latest
```

เช็คว่า API ขึ้นแล้วที่ http://localhost:8887/swagger-ui.html (UI test ไม่ต้องใช้ Docker)
...
| `npm run test:api` | รัน API test ทั้งหมด (POST/GET employees — **ต้องรัน Docker ก่อน**) |
...
resources/
  ...
  datavariables/   # test data — LoginData, ProductData, ShippingData, EmployeeData
pages/             # keyword (action) ของแต่ละหน้า UI — ดึง locator จาก resources/config
api/               # keyword ของ API (EmployeeApi) — Service Object คุยกับ REST API
testscript/        # โค้ดทดสอบ (step definitions) — เรียก keyword จาก pages/api
features/ui/       # ไฟล์ .feature (Gherkin) ของ UI Step 1-4
features/api/      # ไฟล์ .feature ของ API (POST/GET employees)
support/           # world.ts (state ต่อ scenario), hooks.ts (เปิด-ปิด browser/API), config.ts (env)
...
| **`pages/`** | **keyword (action)** ของแต่ละหน้า UI เช่น `login()`, `addToCart()` — ดึง locator จาก `config/` | **Custom Library** (UI) |
| **`api/`** | **keyword ของ API** (`EmployeeApi`: `create()`, `getById()`) — คุยกับ REST API | **Custom Library** (API) |
...
|---|---|---|
| POST | ข้อมูลถูกต้อง -> 201 | email ผิดรูปแบบ -> 400 |
| GET `/{id}` | id ที่มีอยู่ -> 200 | id ที่ไม่มี -> 404 + "Employee not found with ID {id}" |
